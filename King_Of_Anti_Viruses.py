...
def scan(file):
      ...
      if not virus_found:
            print("File is safe!")
            label_status.configure(text="Status: This file is safe!", width = 100, height = 4,  
                            fg = "green")

            scan_md5(file)
      else:
            print("Virus found! File quarentined")
            label_status.configure(text="Status: Virus found! File Deleted!", width = 100, height = 4,  
                            fg = "red")
            os.remove(file)
...
def browseFiles():
    ...
    opened_file.configure(text="File Scaning: "+filename)
...
window.title('King Of Anti Viruses') 
